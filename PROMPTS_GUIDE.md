...
Given an idea, an agent scaffolds. Within minutes there is a framework, a database and a folder of services, and every piece is a decision taken from the first search result. This prompt first works out what it takes to make the idea happen: who needs it, what already exists to build on or stand apart from, the cost to build and run, the funding routes (the owner's own means, pre-orders, revenue, grants, sponsorship, partners, phasing), how the first hundred people will find it, and what success needs. It never concludes that an idea cannot be done. Money, time, physics and law are constraints with routes around them, costed honestly, and the owner chooses the route. It writes the goal as measures with dates, and the course changes that name the next route when the first is blocked. It sorts every decision by what it would cost to reverse. Every recorded decision gets at least two backings, one of them verified. One-way doors (the data model, the platform, the board, a vendor holding the data) get three options, two kinds of evidence, a way out and the owner's approval. It founds every area the product needs, not only engineering, and threads each journey through all of them, so the user flow, the architecture and the data model stay one design. Every stage of the pipeline becomes a command a person, an agent or CI can run, so automation is a dial rather than a rebuild. It gives each stage to the agent whose strengths fit it, and builds the missing tools into the repository. Then it builds one thin slice end to end, plans milestones that each leave something usable with the first release to real people early, and writes the ledger (`PROJECT.md` and `decisions/`) that every later change must trace to.
...
A project is never finished, and it rarely fails on one day. It drifts or stalls: a status report copied forward, a decision whose revisit condition fired unnoticed, a superseded job still running, a free tier that ended, a documented release path that broke while an agent released through its own tool, a first route that hit a wall with no next route written down. This prompt takes every measure again from its source, never from the last report. It switches routes where a course change fired, and reopens decisions whose conditions fired. It finds the code, services, dependencies and spending that nothing serves any more, and routes around costs that outgrew their budget, because money is a constraint to engineer around, not a reason to stop. It runs every pipeline stage through the person's path and the agent's or CI's. It maps what the people using the project said to measures, then ranks the next improvements by what they move, and raises targets once they hold. The course is continue, adjust, re-route or grow, and the project stays able to continue whoever runs it.

**When to use it:**
*   At the end of every milestone, and on a steady cadence after launch: weekly at first, then monthly, and more often while a measure is off target.
*   When a status report says everything is fine and nobody can say where the numbers came from, or when progress has stalled.
