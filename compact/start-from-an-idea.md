# Start a Project from an Idea: short form

To take any idea, software or physical, to a foundation it can grow on without later regret. It works out what it takes to make the idea happen and how that is paid for, makes the costly-to-reverse decisions with verified evidence, gives it a pipeline a person or an agent can run and one slice working end to end, and writes a ledger that gives every later change a reason.

For small or local models with a short context. The full skill, with the reasons and methods behind each rule, is `skills/start-from-an-idea/SKILL.md`; use it whenever the context allows. Works in any harness, needs no hosted service, and sends nothing beyond the project's confidentiality rules.
...
- Ask the owner only what only they know, all at once.
- Work out what it takes to make it happen, before deciding how.
- Write the goal, the measures, and the course changes.
- Sort decisions by what they would cost to reverse, and spend the evidence there.
- Choose every resource's source; do not take the first.
- Weigh several options at once, and sequences of them.
- Found every area the product needs, not only engineering.
- Thread each journey through every area.
- Make every stage of the pipeline a command that a person, an agent or CI can run.
- Route work to agents by what they may see, then by what they have proved they can do.
- Set it up so nobody works in the dark.
- Build a walking skeleton before any breadth.
- Plan milestones that each leave something a person can use, and get it out early.
- Write the ledger, and make CI refuse what it cannot account for.
- Prove every number you rely on.
