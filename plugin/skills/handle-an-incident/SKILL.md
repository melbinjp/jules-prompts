...
description: 'To bring a live product back for the people it failed, then make the
  failure unable to recur. Stop the harm to data, restore by an action already tried,
  tell people and keep the evidence; then prove the cause with a failing test, fix
  it through the normal gates, put everyone right, and close the class of failure
  in the system. Category: Lifecycle.'
license: MIT
metadata:
  prompt_slug: task_handle_an_incident
  source: _prompts/task_handle_an_incident.md
  title: Handle an Incident, Restore First and Then Prevent It
  category: Lifecycle
---

# Handle an Incident, Restore First and Then Prevent It
