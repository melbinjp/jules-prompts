...
description: 'To describe how the system actually works, by deriving it from what
  runs and what imports what rather than from the folder names. Category: Build.'
license: MIT
metadata:
  prompt_slug: task_map_the_architecture
  source: _prompts/task_map_the_architecture.md
...
*   **A verdict table** with one row per claim in the map, and one for the prediction check. Each is `holds` (established by running, resolving or counting), `broken` (the code contradicts the layout or the docs) or `skipped` (could not be established), with the evidence or the reason.
