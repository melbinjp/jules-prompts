...
description: 'To make a consequential choice, such as a vendor, platform, part, provider,
  library or where a resource comes from, against criteria taken from the project''s
  goal. Evidence is in proportion to the stakes, every number is redone, and the choice
  keeps a way out and a condition that reopens it. It can also choose several options,
  or one after another. Category: Lifecycle.'
license: MIT
metadata:
  prompt_slug: task_choose_with_evidence
  source: _prompts/task_choose_with_evidence.md
  title: Choose Between Options with Evidence
  category: Lifecycle
---

# Choose Between Options with Evidence
...
*   **Set the stakes first, and let them set the effort.**
    ...
    *   **What each door needs:**
        *   A two-way door needs two options, one piece of evidence and a seam. It is an hour's work, not a week's.
...
*   **Take evidence from primary sources and from your own measurement.**
    ...
    *   **For a one-way door, get two independent kinds of evidence:** a calculation and a measurement, for example, or a primary source and a prototype. Two articles quoting the same benchmark are one piece of evidence.
*   **Redo every number.**
...
*   **Record it.**
    ...
    *   **The check:** jules-prompts publishes a trace check, `harness/check_trace.py`. It counts the options and the evidence, and refuses a one-way door with no exit or no approval.
...
*   **A verdict table** with one row per item:
    ...
    *   the evidence independent, with two kinds for a one-way door;
    *   every number redone;
