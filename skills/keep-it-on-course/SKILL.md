...
description: 'To review a project against its own ledger at each milestone and after
  launch. Every measure is taken again from its source, every stop and revisit condition
  is checked, whatever nothing serves is removed, and both a person and an agent run
  the pipeline. It ends in a decision, with evidence, to continue, adjust, pivot,
  pause or stop. Category: Lifecycle.'
license: MIT
metadata:
...
# Keep a Project on Course, During and After Launch
