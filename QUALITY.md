...
- **Budgets:** stylesheet under 16 KB, script under 8 KB, no web font, preview image under 150 KB, every page under 120 KB. No script, stylesheet or font from another origin; only the shared favicon.
...
- **Legal:** MIT. No web font: text is set in the reader's own system font. The name began as prompts for Google's Jules; the site says it is not affiliated with Google, in the footer and in its questions.
...
| agent, from the discovery index | 26 of 26 `SKILL.md` served byte for byte, digests match (CI) | holds |
...
| MCP client with no network at all | `JULES_PROMPTS_DIR` serves all 26 from a copy; in that mode fetch throws, and a planted network call made the server refuse to start (CI) | holds |
| small local model | a short form of each of the 26 skills in `compact/`, 291 to 945 words, generated and checked byte for byte (CI) | holds |
...
| person, desktop, light and dark | screenshots of home, skills, a skill, workflow | holds |
...
| person, keyboard | skip link is the first stop and moves focus to the content; every stop has a 2 px outline in the link colour | holds |
| contrast, WCAG AA | computed for every token pair on both surfaces (page and panel). Light: text 17.1, secondary 6.8, links 7.2, verdict colours at least 5.0. Dark: text 15.4, secondary 7.1, links 8.0, verdict colours at least 7.3. Button text 17.1 and 15.4 | holds |
...
| every skill can go red | 26 fixtures, 146 planted defects, one fixture for every skill (ten had none, while the home page said each had one): each expected report names all of its own, and the scorer exits 1 on a report that misses one. The integrity check refuses a skill without a fixture, and one without the standard sections or a closing verdict with its denominator; both seen failing on a planted break (CI) | holds |
| budgets | stylesheet 12.3 KB (18.9 KB before the site was made quieter), script 3.3 KB, no web font (a 23.6 KB display font before; the site check now refuses one), preview image 51.6 KB, largest page 39 KB, `start-from-an-idea` (CI) | holds |
...
| Claude Code plugin install | from a clean config, marketplace add and install: 26 skills, about 2.4k tokens in every session (3.0k with the 36 before the ten superseded skills were removed; 3.5k when every skill was listed twice). The longest skill, `start-from-an-idea`, costs about 7.3k when loaded, and `run-autonomously` about 5.3k; their short forms in `compact/` are about 1.3k each. Both manifests pass `claude plugin validate --strict` | holds |
...
| a saved link to a removed page | each of the 12 removed pages redirects to the page that now does its job, generated from one map (`MOVED` in `scripts/emit.py`) that refuses a target that does not exist; none is in the sitemap | holds |
